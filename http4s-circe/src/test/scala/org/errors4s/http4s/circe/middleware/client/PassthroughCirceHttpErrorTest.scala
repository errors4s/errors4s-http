...
final class PassthroughCirceHttpErrorTest extends CatsEffectSuite {
  ...
  private def compareResponses(value: Response[SyncIO], expected: Response[SyncIO]): SyncIO[Unit] =
    SyncIO(assertEquals(value.status, expected.status)) *> SyncIO(assertEquals(value.headers, expected.headers)) *>
      bodyAsChunk(value)
        .flatMap(value => bodyAsChunk(expected).flatMap(expected => SyncIO(assertEquals(value, expected)).void))
        ...
        e =>
          e match {
            case e: ExtensibleCirceHttpProblem =>
              SyncIO(assertEquals(e.asJson, testError.asJson))
            case otherwise =>
    ...
    Ref
      ...
      .flatMap { ref =>
        ...
        client.run(testRequest).use(value => compareResponses(value, response)) *>
          ref.get.flatMap(value => SyncIO(assertEquals(value, 2)))
      }
  }
    ...
    Ref
      ...
      .flatMap { ref =>
        ...
        client
          ...
          ) *> ref.get.flatMap(value => SyncIO(assertEquals(value, 0)))
      }
  }
}
